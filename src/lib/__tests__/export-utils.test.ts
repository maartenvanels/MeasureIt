...
import { generateCSV, generateJSON, generateClipboardText } from '../export-utils';
...
describe('generateCSV', () => {
  ...
  it('includes reference measurement', () => {
    const csv = generateCSV([ref], 100, 'mm', ref);
    expect(csv).toContain('"Reference"');
    expect(csv).toContain('reference');
  });

  it('includes regular measurement with real value', () => {
    const csv = generateCSV([ref, measure], 100, 'mm', ref);
  ...
  it('includes angle measurement', () => {
    const csv = generateCSV([angle], 100, 'mm');
...
describe('generateJSON', () => {
  it('generates valid JSON', () => {
    const json = generateJSON([ref, measure], 100, 'mm', ref);
  ...
  it('includes measurement coordinates', () => {
    const json = generateJSON([measure], 100, 'mm', ref);
    const parsed = JSON.parse(json);
  ...
  it('includes angle data', () => {
    const json = generateJSON([angle], 100, 'mm');
    const parsed = JSON.parse(json);
  ...
  it('includes area data', () => {
    const json = generateJSON([area], 100, 'mm', ref);
    const parsed = JSON.parse(json);
  ...
  it('includes annotation data', () => {
    const json = generateJSON([annotation], 100, 'mm');
    const parsed = JSON.parse(json);
  ...
  it('returns null realValue without reference', () => {
    const json = generateJSON([measure], 100, 'mm');
    const parsed = JSON.parse(json);
